...
const Home: React.FC<{}> = () => {
    ...
    const handleInputBlur = (e: any) => {
        ...
        demoLiValue[nowDoubleClick] = { name: e.target.value, value: e.target.value }; // 根据ul绑定的事件代理，找到正在修改的节点序号，然后更改数组对应的数据
        ...
        demoLiValue?.map((item: any, index: number) => (
            <li key={index}>
                <div className="view" ref={divRef}>
                    ...
                    />
                    <label onDoubleClick={e => handleLabelDoubleClick(e)}>{item.name}</label>
                    <button
                        className="destroy"
                        value={item.name}
                        onClick={e => {
                            ...
                            setdemoLiValue(
                                demoLiValue.filter((item: any) => item.name !== e.target['value'])
                            );
                        }}
                    ></button>
                </div>
